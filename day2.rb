...
def part1(passwords)
  ...
  passwords.each do |password|
    ...
    counts = rules[0].split('-')
    character = rules[1][0, 1]
    string = rules[2]

    if string.count(character) >= counts[0].to_i &&
       string.count(character) <= counts[1].to_i
      valid_passwords += 1
    end
  end
...
def part2(passwords)
  ...
  passwords.each do |password|
    ...
    counts = rules[0].split('-')
    first = counts[0].to_i - 1
    second = counts[1].to_i - 1
    character = rules[1][0, 1]
    string = rules[2]

    next if string[first, 1] == character && string[second, 1] == character

    if (string[first, 1] == character && string[second, 1] != character) ||
       (string[first, 1] != character && string[second, 1] == character)
      valid_passwords += 1
    end
  end
