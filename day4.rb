...
def part1(passports)
  ...
  parsed_passports = []
  parsed_passport = []
  passports.each.with_index do |passport, index|
    if passport != '' && index + 1 == passports.length
      parsed_passport << passport
      parsed_passports << parsed_passport
    elsif passport != ''
      parsed_passport << passport
    else
      parsed_passports << parsed_passport
  ...
  parsed_passports.each do |passport|
    passport_keys = passport.join(' ').split(' ').map { |key_values| key_values.split(':')[0] }
    valid_passports += 1 if (required_keys - passport_keys).empty?
  end
...
def part2(passports)
  ...
  parsed_passports.each do |passport|
    passport_keys = passport.join(' ').split(' ').map { |key_values| key_values.split(':')[0] }
    next unless (required_keys - passport_keys).empty?

    passport_hash = passport.join(' ').split(' ').map { |data| data.split(':') }.to_h

    # 1920 byr 2002
    next if passport_hash['byr'].to_i < 1920 || passport_hash['byr'].to_i > 2002

    # 2010 iyr 2020
    next if passport_hash['iyr'].to_i < 2010 || passport_hash['iyr'].to_i > 2020

    # 2020 eyr 2030
    next if passport_hash['eyr'].to_i < 2020 || passport_hash['eyr'].to_i > 2030

    # hgt ends with cm or in
    next if passport_hash['hgt'][-2, 2] != 'cm' && passport_hash['hgt'][-2, 2] != 'in'

    # 150cm hgt 193cm
    next if passport_hash['hgt'][-2, 2] == 'cm' &&
            (passport_hash['hgt'][0, passport_hash['hgt'].length - 2].to_i < 150 ||
            passport_hash['hgt'][0, passport_hash['hgt'].length - 2].to_i > 193)

    # 59in hgt 76in
    next if passport_hash['hgt'][-2, 2] == 'in' &&
            (passport_hash['hgt'][0, passport_hash['hgt'].length - 2].to_i < 59 ||
            passport_hash['hgt'][0, passport_hash['hgt'].length - 2].to_i > 76)

    # hcl /#\w{6}/
    next unless /^#\w{6}$/.match(passport_hash['hcl'])

    # ecl - amb blu brn gry grn hzl oth
    next unless %w[amb blu brn gry grn hzl oth].include?(passport_hash['ecl'])

    # pid /#\d{9}/
    next unless /^\d{9}$/.match(passport_hash['pid'])
